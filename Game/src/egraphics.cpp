...
namespace EProject
{
    ...
    RenderPass::RenderPass(const GDevicePtr& ptr) : DeviceHolder(ptr)
    {

    }

    GeometryPass::GeometryPass(const GDevicePtr& ptr) : RenderPass(ptr)
    {
    ...
    {
        m_shaderProgram = m_device->createShaderProgram();
        ...
        m_shaderProgram->compileFromFile(vsInputTri);
        m_shaderProgram->compileFromFile(psInputTri);
    ...
    void GeometryPass::passLight(const DirectLightComponent& dirLight)
    {
        m_dirLight = dirLight;
    }

    void GeometryPass::execute(const CameraBase* cameraPtr, const PassMeshInfo& meshInfo)
    {
        const auto& mesh = meshInfo.mshPtr.m_model;
        const auto& transform = meshInfo.trsPtr;
        const auto mdlMatrix = transform.getModelMatrixWorld();
        const auto inverseMdlMatrix = transform.getInverseModelMatrixWorld();
        
        m_shaderProgram->setResource(m_shaderSemanticsc.at("samplerDefault"), cSampler_Linear);

        m_shaderProgram->setResource(m_shaderSemanticsc.at("albedoTexture"), mesh->getAlbedoTexturePtr());
        m_shaderProgram->setResource(m_shaderSemanticsc.at("normalTexture"), mesh->getNormalTexturePtr());
        m_shaderProgram->setResource(m_shaderSemanticsc.at("metallRoghnessTexture"), mesh->getMetallRoughnessTexturePtr());

        m_shaderProgram->setValue(m_shaderSemanticsc.at("modelMatrix"), mdlMatrix);
        m_shaderProgram->setValue(m_shaderSemanticsc.at("invModelMatrix"), inverseMdlMatrix);

        const Camera3D* camera3D = static_cast<const Camera3D*>(cameraPtr);
        m_shaderProgram->setValue(m_shaderSemanticsc.at("cameraPos"), camera3D->getPosition());
        m_shaderProgram->setValue(m_shaderSemanticsc.at("viewProjectionMatrix"), camera3D->getViewProj());

        m_shaderProgram->setValue(m_shaderSemanticsc.at("lightPositions"), m_dirLight.mPos);
        m_shaderProgram->setValue(m_shaderSemanticsc.at("lightColours"), m_dirLight.mColor);

        m_shaderProgram->setInputBuffers(mesh->getVertexBufferPtr(), mesh->getIndexBufferPtr(), {}, 0);
        m_shaderProgram->drawIndexed(PrimTopology::Triangle, 0, mesh->getIndexBufferPtr()->getIndexCount());
    }

    void GeometryPass::createShaderSemantics()
    {
    ...
    Render3D::Render3D(const GDevicePtr& _dev, const Camera3DPtr& camera) : DeviceHolder(_dev),
        m_cam3DPtr(camera), 
        m_geoPassPBR(_dev)
    {
    ...
    {
        m_geoPassPBR.passLight(dirLight);
    }

    void Render3D::drawMeshModel(const StaticMeshComponent& mshPtr, const TransformComponent& trs)
    {
        PassMeshInfo passInfo = { mshPtr, trs };
        m_geoPassPBR.execute(m_cam3DPtr.get(), passInfo);
    }
