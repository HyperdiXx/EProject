...
namespace EProject
{
    ...
    struct PassMeshInfo
    {
        StaticMeshComponent mshPtr;
        TransformComponent trsPtr;
    };

    class RenderPass : public DeviceHolder
    {
    public:
        RenderPass(const GDevicePtr& ptr);
        virtual void init() = 0;
        virtual void execute(const CameraBase* cameraPtr, const PassMeshInfo& meshInfo) = 0;
    ...
    {
    public:
        GeometryPass(const GDevicePtr& ptr);

        void init() override;
        void execute(const CameraBase* cameraPtr, const PassMeshInfo& meshInfo) override;
        void shutdown() override;
        void passLight(const DirectLightComponent& dirLight);
    private:
        void createShaderSemantics();
    private:
        ShaderProgramPtr m_shaderProgram;
        StructuredBufferPtr m_strBuffer;
    ...
    private:
        GeometryPass m_geoPassPBR;
        Camera3DPtr m_cam3DPtr;

        std::shared_ptr<AssetManager> m_assetMng;
    };
}
